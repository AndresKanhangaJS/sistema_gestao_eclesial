<x-filament-panels::page>
    ...
    no seletor acima a tabela por baixo actualiza (e computed na propria
    pagina) mas os graficos ficavam presos ao ano do primeiro carregamento —
    a key fixa impede o Livewire de os remontar.
            ...
            <tbody>
                @foreach (['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'] as $i => $mesLabel)
                    ...
                    <tr class="border-t border-gray-100 dark:border-gray-700">
                        ...
                        @foreach ($this->dados['categorias'] as $categoria)
                            <td class="px-3 py-2 text-right">{{ number_format($linha[$categoria['chave']], 2, ',', '.') }}</td>
                        @endforeach
                ...
                <tr class="border-t-2 border-gray-300 dark:border-gray-600 font-bold">
                    ...
                    @foreach ($this->dados['categorias'] as $categoria)
                        <td class="px-3 py-2 text-right">{{ number_format($this->dados['por_categoria'][$categoria['chave']], 2, ',', '.') }}</td>
                    @endforeach
